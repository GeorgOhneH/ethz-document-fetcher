...
class DynamicIcon(QIcon):
    def __init__(self, path):
        ...
        QApplication.instance().theme_changed.connect(self.set_icon)
...
class DynamicIconLabel(QLabel):
    def __init__(self, path, width, height, parent):
        ...
        QApplication.instance().theme_changed.connect(self.set_pixmap)

    def _set_pixmap(self, path):
        icon = QIcon(path)

        device_ratio = qApp.devicePixelRatio()
        pixmap = icon.pixmap(QSize(self.icon_width * device_ratio, self.icon_height * device_ratio))
